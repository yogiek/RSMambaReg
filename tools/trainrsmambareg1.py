...
from tools.ewc import ElasticWeightConsolidation
...
def main():
    ...
    # Tambahkan Elastic Weight Consolidation
    ewc = ElasticWeightConsolidation(
        model=runner.model, 
        lambda_ewc=cfg.get('lambda_ewc', 1000), 
        device='cuda' if torch.cuda.is_available() else 'cpu'
    )

    # Jika ingin menggunakan Fisher Matrix dari model sebelumnya
    if cfg.get('ewc_load', False):
        ewc.save_params()
        ewc.compute_fisher_matrix(runner.train_dataloader)
    
    criterion = nn.CrossEntropyLoss()
    criterion = CustomEWCLoss(base_loss=criterion, ewc=ewc)
    runner.model.head.loss = criterion
