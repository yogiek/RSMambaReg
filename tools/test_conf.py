...
import sys
sys.path.append("/home/cluster-dgxa100/yogiekindra/RSMambaReg/mmpretrain")
import argparse
import os
...
class Accuracy(BaseMetric):
    ...
    def __init__(self, topk=(1,), collect_device='cpu', prefix=None):
        ...
        self.results = []  # Inisialisasi results sebagai list kosong
    ...
    def compute_metrics(self, results):
        ...
        return {f'{self.prefix}/top1': accuracy}


class ConfusionMatrixMetric(BaseMetric):
    """Menyimpan confusion-matrix sebagai gambar (.jpg)."""
    ...
    def __init__(self,
                 ...
                 file_name: str = 'confusion_matrix.jpg',
                 collect_device: str = 'cpu',
                 prefix: str | None = None):
        super().__init__()
        self.num_classes = num_classes
        self.class_names = class_names
        self.normalize = normalize          # None, 'true', 'pred', atau 'all'
        self.file_name = file_name
        self.collect_device = collect_device
        self.prefix = prefix if prefix else self.default_prefix
        self.results = []                   # (pred, label)
    ...
    def compute_metrics(self, results):
        ...
        # dikembalikan supaya runner tidak error, meskipun nilainya hanya lokasi file
        return {f'{self.prefix}/path': self.file_name}
...
def main():
    ...
    # Tambahkan evaluator untuk akurasi
    accuracy_evaluator = Accuracy(topk=(1,))  # Top-1 accuracy
    runner.test_evaluator.metrics.append(accuracy_evaluator)

    # ---------------- Confusion-matrix ----------------
    num_classes = cfg.model.head.num_classes  # =21 pada rsmamba_uc_b.py
    # Contoh label kelas (ganti dengan nama kelas sebenarnya agar lebih informatif)
    class_names = [f'class_{i}' for i in range(num_classes)]

    cm_path = osp.join(cfg.work_dir, 'confusion_matrix.jpg')
    confusion_evaluator = ConfusionMatrixMetric(
        num_classes=num_classes,
        class_names=class_names,
        normalize=None,            # ubah ke 'true' bila ingin normalized
        file_name=cm_path)

    runner.test_evaluator.metrics.append(confusion_evaluator)

    # start testing
    metrics = runner.test()


    if 'accuracy/top1' in metrics:
        print(f"Accuracy: {metrics['accuracy/top1']:.4f}%")
